...
def test_model(X_test, Y_test, model, comm_round):
    ...
    with torch.no_grad():
        ...
        # Convert tensors to numpy arrays for metric functions
        y_true = Y_test.cpu().numpy()
        y_pred = Y_prdt.cpu().numpy()
        y_scores = logits.cpu().numpy()

        # Compute common metrics using sklearn functions
        acc = accuracy_score(y_true, y_pred)
        F1 = f1_score(y_true, y_pred, zero_division=1)
        precision = precision_score(y_true, y_pred, zero_division=1)
        recall = recall_score(y_true, y_pred, zero_division=1)
        
        # Compute confusion matrix to get TN, FP, FN, TP for additional metrics
        ...
        # Calculate specificity as an additional metric: TN / (TN + FP)
        specificity = TN / (TN + FP) if (TN + FP) > 0 else 0

        # ROC and AUC calculations
        fpr_curve, tpr_curve, thresholds = metrics.roc_curve(y_true, y_scores)
        auc_value = roc_auc_score(y_true, y_scores)

        # Print all desired metrics including the added ones
        print('comm_round: {} | acc: {:.3%} | loss: {:.4f} | F1: {:.4f} | precision: {:.4f} | recall: {:.4f} | AUC: {:.4f} | FPR: {:.4f} | specificity: {:.4f}'.format(
            comm_round, acc, loss.item(), F1, precision, recall, auc_value, FPR, specificity))
            
    return acc, loss.item(), F1, precision, recall, auc_value, FPR, specificity
