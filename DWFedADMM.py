...
def calculate_dynamic_weight(acc_i_r_minus_1, best_acc_r_minus_1):
    # Dynamic weight for client i at round r based on its accuracy
    return acc_i_r_minus_1 / best_acc_r_minus_1 if best_acc_r_minus_1 > 0 else 1.0  # Prevent division by zero


# Update client-side operations with dynamic weighting
def update_local_model_with_dynamic_weight(client_model, global_model, accuracy_i_r_minus_1, best_accuracy_r_minus_1):
    # Calculate dynamic weight for the client based on its accuracy
    omega_i_r = calculate_dynamic_weight(accuracy_i_r_minus_1, best_accuracy_r_minus_1)

    # Perform weighted averaging between the client's model and global model
...
# Assuming you have a method to test the model and calculate accuracy
def evaluate_client_accuracy(client_model, X_test, y_test):
    client_model.eval()  # Switch model to evaluation mode
    with torch.no_grad():
        outputs = client_model(X_test)
        predicted = (outputs > 0.5).float()  # Binary classification (0 or 1)
        accuracy = accuracy_score(y_test, predicted)
    return accuracy

def track_best_accuracy(client_accuracies):
    best_accuracy = max(client_accuracies)
